...
- **Widget opacity:** `WidgetOpacity` in `Global.inc` (percent, default `100`) is a global whole-widget transparency. `Widget.inc`'s `[OpacityValue]` Calc measure clamps it to 10–100%, scales it to a 0–255 alpha, and applies it to each widget's window with `!SetTransparency` on every load/refresh — so all widgets fade uniformly (card, text, icons) while the settings panel and popups stay opaque. The Appearance tab's *Advanced Options* exposes it beside Widget size/padding/radius.
