...
- **Appearance state** — `Palette` (default `Light`), `Theme` as the effect selector (default `Blur`), `DarkMode`, `BatterySaverMode`, the `DarkThemeFrom`/`DarkThemeTo` schedule (`23:00`–`09:00`), and `WidgetOpacity` (background-card opacity percent, default `100`) with its derived `WidgetBackgroundAlpha` (see [[Widget Scaffold]]).
